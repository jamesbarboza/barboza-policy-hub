-- Find admin user by email and use their ID for policy creation
DO $$
DECLARE
    admin_user_id uuid;
BEGIN
    -- Try to find the admin user with email admin@barboza.com
    -- We need to join with auth.users to get the email, but since that's not directly accessible
    -- in migrations, we'll look for an admin role user first
    ...
    -- In production, ensure the admin@barboza.com user exists and has admin role
    IF admin_user_id IS NULL THEN
        -- Use a consistent UUID for the admin user
        admin_user_id := '00000000-0000-0000-0000-000000000001';
        
        -- Insert admin role for this user (assuming they will be created)
